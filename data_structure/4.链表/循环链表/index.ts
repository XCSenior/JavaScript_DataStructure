 ...
 */
/**
 * @class SingleNode2 普通双向链表封装的结点
 * @property { T } data 结点的数据
 * @property { SingleNode2<T> | null } prev 结点的直接前驱
 * @property { SingleNode2<T> | null } next 结点的直接后继
 * @description 正常来说应该继承SingleNode类的，但是由于ts编译后的commonJS模块化格式浏览器不支持（不知道怎么设置编译后成为ESModules），所以还是单独声明此双向链表结点的类
 */
class SingleNode2<T> {
    public data: T;
    public prev: SingleNode2<T> | null;
    public next: SingleNode2<T> | null;
    constructor(data: T){
        this.data = data;
        this.prev = null;
        this.next = null;
...
class CircularLinkedList<T> {
    ...
    // 单链表的表头，表尾元素
    public head: SingleNode2<T> = null;
    public tail: SingleNode2<T> = null;

    // 编写公用方法
    // 公用方法1、获取指定位置的结点
    public getNodeAt(targetIndex: number): SingleNode2<T> | void{
        if (targetIndex >= 0 && targetIndex < this.count) {
            let currentNode: SingleNode2<T> = this.head;
            // 开始遍历查找目标节点
    ...
    // 1、插入元素至链尾
    public push(data: T){
        const node = new SingleNode2(data);
        if (this.head === null) {
            this.head = node;
            this.tail = node;
        } else {
    ...
    public removeAt(targetIndex: number): T | void{
        if (targetIndex >= 0 && targetIndex < this.count) {
            let deleteTargetNode: SingleNode2<T>;
            if (targetIndex === 0) {
                /**
                 * 应该考虑到
                ...
                deleteTargetNode = <SingleNode2<T>>this.getNodeAt(targetIndex);
    ...
    public insertTo(targetIndex: number, data: T): boolean{
        if (targetIndex >= 0 && targetIndex < this.count) {
            const newNode = new SingleNode2(data);
            /* 分头尾的情况 */
            if (targetIndex === 0) {
                if (this.head) {
                ...
                const targetNode = <SingleNode2<T>>this.getNodeAt(targetIndex - 1);
    ...
    // 8、（7）获取链表头head
    public getHead(): SingleNode2<T> {
        return this.head;
    }
    // 9、返回链表尾
    public getTail(): SingleNode2<T> {
        return this.tail;
    }
}
